...
Sliding window dzieli długie nagrania na okna po 30 klatek (1s @ 30fps).
"""
...
class ExerciseDataset(Dataset):
    ...
    i etykieta (int) — 0=correct, 1-4=typy błędów.
    """
...
def load_dataset(exercise: str = ACTIVE_EXERCISE,
    ...
    Obsługuje dwa formaty katalogów:
    A) Prosty (binary): correct/ + error/
    B) Multi-class: correct/ + knee_valgus/ + heel_lift/ + ...

    Args:
        exercise: Nazwa ćwiczenia (np. "pushup", "squat", "lunge")
    ...
    for label_id, label_name in labels_config.items():
        ...
        if npy_files:
            icon = "✅" if label_id == 0 else "❌"
            print(f"  {icon} {label_name}: {len(npy_files)} plików")

    if len(all_sequences) == 0:
        raise FileNotFoundError(
            f"Brak plików .npy w {exercise_dir}. "
            f"Uruchom najpierw extract_keypoints.py!"
        )

    # Statystyki
    print(f"\n📊 Łącznie: {len(all_sequences)} próbek")
    for label_id, label_name in labels_config.items():
